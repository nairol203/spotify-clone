		...
		<div className='mt-8 grid gap-4 md:py-4'>
			<div className='flex items-center gap-4 md:gap-6 lg:gap-8'>
				<div className='relative h-32 w-32 md:h-44 md:w-44 lg:h-60 lg:w-60 aspect-square'>
					<Image src={album.data.images?.[0]?.url} alt={`Album Cover from ${album.data.name}`} fill sizes='8rem 11rem 15rem' className='object-cover aspect-square' />
				</div>
				<div className='grid gap-2 md:gap-4'>
					<span className='capitalize'>{album.data.album_type}</span>
					<h1 className='text-2xl sm:text-4xl md:text-4xl lg:text-6xl whitespace-nowrap overflow-hidden text-ellipsis'>{album.data.name}</h1>
					<div className='flex gap-1.5 text-xs sm:text-sm md:text-base'>
			...
			<div>
				...
				{album.data.tracks.items.map((track, index) => (
					<div className='grid grid-cols-[1.25rem_6fr_1fr] items-center gap-4 rounded-[4px] px-4 py-2 md:hover:bg-white md:hover:bg-opacity-10' key={track.id}>
						...
						<div className='flex items-center gap-4 whitespace-nowrap overflow-hidden text-ellipsis'>
							...
							<div className='overflow-hidden text-ellipsis'>
								<Link className='hover:underline overflow-hidden text-ellipsis' href={`/track/${track.id}`}>
