...
export default function User() {
    ...
    const profile = trpc.profile.useQuery({ user_id: router.query['id']?.[0] as string });
        ...
        <div className='mt-8 grid gap-4 md:py-4'>
            <div className='flex items-center gap-4 md:gap-6 lg:gap-8'>
                <div className='relative h-32 w-32 md:h-44 md:w-44 lg:h-60 lg:w-60'>
                    <Image src={profile.data.images?.[0]?.url ?? ''} alt='Playlist Cover' fill sizes='8rem 11rem 15rem' className='rounded-full object-cover' />
                </div>
                <div className='grid gap-2 md:gap-4'>
